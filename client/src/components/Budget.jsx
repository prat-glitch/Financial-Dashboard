...
const CategoryBudgetCard = ({ categoryName, budget, spent, onChange, onDelete, isDarkMode }) => {
  ...
  return (
    <div className={`
      group rounded-xl sm:rounded-2xl p-4 sm:p-5 border-l-4 transition-all duration-200
...
const SavingsCard = ({ title, value, subtitle, icon: Icon, color, isDarkMode, isMain = false }) => {
  ...
  return (
    <div className={`
      rounded-xl sm:rounded-2xl p-4 sm:p-5 border-l-4 transition-all duration-200
      hover:-translate-y-0.5 hover:shadow-lg
      ${isDarkMode 
    ...
    `}>
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        `}>
          <Icon size={18} className={`${isDarkMode ? 'text-violet-400' : style.text} sm:w-[22px] sm:h-[22px]`} />
        </div>
        <div>
